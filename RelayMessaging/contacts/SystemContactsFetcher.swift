...
class ContactsFrameworkContactStoreAdaptee: NSObject, ContactStoreAdaptee {
    ...
    func runChangeHandler() {
        guard let changeHandler = self.changeHandler else {
            owsFailDebug("\(self.logTag) trying to run change handler before it was registered")
            return
        }
        changeHandler()
    }
    ...
    func fetchContacts() -> Result<[Contact], Error> {
        ...
        do {
            ...
            owsFailDebug("\(self.logTag) Failed to fetch contacts with error:\(error)")
            return .error(error)
        }
        ...
        do {
            ...
            try self.contactStore.enumerateContacts(with: contactFetchRequest) { (contact, _) -> Void in
                guard result == nil else {
                    owsFailDebug("\(self.logTag) More than one contact with contact id.")
                    return
                }
                result = contact
            }
        } catch let error as NSError {
            owsFailDebug("\(self.logTag) Failed to fetch contact with error:\(error)")
            return nil
        }
        ...
        case .notDetermined:
            ...
            self.contactStoreAdapter.requestAccess { (granted, error) in
                ...
                guard granted else {
                    ...
                    owsFailDebug("\(self.logTag) declined contact access.")
                    completion(nil)
                    return
                }
