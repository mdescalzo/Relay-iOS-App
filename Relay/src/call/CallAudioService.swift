...
@objc class CallAudioService: NSObject, CallObserver {
    ...
    public func requestSpeakerphone(isEnabled: Bool) {
        ...
        DispatchQueue.global().async {
            do {
                ...
                owsFailDebug("\(self.logTag) failed to set \(#function) = \(isEnabled) with error: \(error)")
            }
        }
    }
    ...
    private func ensureProperAudioSession(call: RelayCall?) {
        ...
        do {
            ...
            owsFailDebug("\(self.logTag) failed setting audio source with error: \(error) isSpeakerPhoneEnabled: \(call.isSpeakerphoneEnabled)")
        }
    }
