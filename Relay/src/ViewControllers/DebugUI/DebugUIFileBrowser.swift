    ...
    func buildContents() -> OWSTableContents {
        ...
        if isDirectory {
            ...
            let directoryContents: [URL] = {
                do {
                    ...
                    owsFailDebug("\(self.logTag) contentsOfDirectory(\(fileURL) failed with error: \(error)")
                    return []
                }
            }()
                ...
                let fileIcon: String = {
                    do {
                        guard let isDirectory = try fileInDirectory.resourceValues(forKeys: Set(resourceKeys)).isDirectory else {
                            owsFailDebug("\(logTag) unable to check isDirectory for file: \(fileInDirectory)")
                            return ""
                        }

                        return isDirectory ? "📁 " : ""
                    } catch {
                        owsFailDebug("\(logTag) failed to check isDirectory for file: \(fileInDirectory) with error: \(error)")
                        return ""
                    }
                }()
        ...
        let attributeItems: [OWSTableItem] = {
            do {
                ...
                owsFailDebug("\(logTag) failed getting attributes for file at path: \(fileURL)")
                return []
            }
        }()
            ...
            OWSTableItem.disclosureItem(withText: "❌ Delete") { [weak self] in
                ...
                OWSAlerts.showConfirmationAlert(title: "Delete \(strongSelf.fileURL.path)?") { _ in
                    ...
                    do {
                        ...
                        owsFailDebug("\(strongSelf.logTag) failed to remove item: \(strongSelf.fileURL) with error: \(error)")
                    }
                }
            },
            ...
            OWSTableItem.disclosureItem(withText: "🔒 Set File Protection") { [weak self] in
                ...
                let currentFileProtection: FileProtectionType? = {
                    do {
                        ...
                        owsFailDebug("\(strongSelf.logTag) failed to get current file protection for file: \(fileURL)")
                        return nil
                    }
                }()
                ...
                protections.forEach { (protection: FileProtectionType) in
                    actionSheet.addAction(UIAlertAction(title: "\(protection.rawValue.replacingOccurrences(of: "NSFile", with: ""))", style: .default) { (_: UIAlertAction) in
                        ...
                        do {
                            ...
                            owsFailDebug("\(strongSelf.logTag) failed to update file protection at path:\(fileURL.path) with error: \(error)")
                        }
                    })
                }
            ...
            let createDirItem = OWSTableItem.disclosureItem(withText: "📁 Create Dir in this Dir") { [weak self] in
                ...
                alert.addAction(UIAlertAction(title: "Create", style: .default) { _ in
                    ...
                    do {
                        ...
                        owsFailDebug("\(strongSelf.logTag) Failed to create dir: \(newPath) with error: \(error)")
                    }
                })
