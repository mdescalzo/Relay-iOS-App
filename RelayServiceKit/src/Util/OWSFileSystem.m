...
+ (void)logAttributesOfItemAtPathRecursively:(NSString *)path
{
    ...
    if (!exists) {
        OWSFailDebug(@"%@ error retrieving file attributes for issing file", self.logTag);
        return;
    }
...
+ (nullable NSError *)renameFilePathUsingRandomExtension:(NSString *)oldFilePath
{
    ...
    if (!success || error) {
        OWSFailDebug(@"%@ Could not move file or directory from: %@ to: %@, error: %@",
            self.logTag,
            oldFilePath,
            newFilePath,
            error);
...
+ (nullable NSError *)moveAppFilePath:(NSString *)oldFilePath sharedDataFilePath:(NSString *)newFilePath
{
    ...
    if ([fileManager fileExistsAtPath:newFilePath]) {
        OWSFailDebug(@"%@ Can't move file or directory from: %@ to: %@; destination already exists.",
            self.logTag,
            oldFilePath,
            newFilePath);
